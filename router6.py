...
def generate_forwarding_table_with_range(table):
    ...
    for row in table:
        ...
        if row[0] != "0.0.0.0":
            ...
            # 7. Build the new row.
            new_row = [ip_range[0], ip_range[1], row[2], row[3]]
...
def ip_to_bin(ip : str):
    ...
    ip_int = int(ip_bin_string, base=2)
...
def receive_packet(connection : socket.socket, max_buffer_size):
    ...
    write_to_file("output/recieved_by_router_6.txt", decoded_packet)
...
def start_server():
    ...
    while True:
        ...
        ip, port = "127.0.0.1", "8006"
        print("Connected with ", ip, ":", port)
        # 9. Start a new thread for receiving and processing the incoming packets.
        try:
            thd = Thread(target=processing_thread, args=(connection, ip, port, 
